...
export const loadJobReducer = (state = { jobs: [], setUniqueLocation: [] }, action) => {
  switch (action.type) {
    case JOB_LOAD_REQUEST:
      return { ...state, loading: true }; // Preserve existing state
    ...
    case JOB_LOAD_FAIL:
      return {
        ...state, // Preserve existing state in case of failure
        loading: false,
        error: action.payload,
      };
    case JOB_LOAD_RESET:
      return { jobs: [], setUniqueLocation: [] }; // Clear to default state on reset
    default:
      return state;
  }
};
